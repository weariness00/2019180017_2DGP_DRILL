...
class TileBackground:
    ...
    def draw(self):
        ...
        tile_left = self.window_left // self.sizeX
        tile_right = (self.window_left + self.canvas_width) // self.sizeX

        tile_bottom = self.window_bottom // self.sizeY
        tile_top = (self.window_bottom + self.canvas_height) // self.sizeY

        offsetX = self.window_left % self.sizeX
        offsetY = self.window_bottom % self.sizeY

        for ty in range(tile_bottom, tile_top + 1):
            for tx in range(tile_left, tile_right + 1):
                self.tiles[ty][tx].draw_to_origin(-offsetX + (tx - tile_left) * self.sizeX,
                                                  -offsetY + (ty - tile_bottom) * self.sizeY)
        pass
